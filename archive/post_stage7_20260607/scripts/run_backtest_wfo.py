...
def run_wfo(
    ...
    # ── 子系统初始化 ──────────────────────────────────────────────────────
    # Stage 7: 单轨 V3.1, PPO 控 theta. config.event_track_version 仅作 telemetry
    # 记录,实际只走 V31Engine 路径.
    from src.compute.v31_engine import V31Engine
    v31_engine = V31Engine(config)
    et_version = config.get("event_track_version", "v3_1")
    log.info(f"[Stage 7] single-track V3.1 -> {type(v31_engine.event_track).__name__}")
    ...
    for i, friday in enumerate(fridays):
        ...
        try:
            ...
            # Stage 7: 单轨 V3.1, PPO 控 theta. theta 在 (D) 步前算.
            # 临时占位 theta,实际在 (D) 步 PPO 推理后会覆盖
            w_event = v31_engine.compute(
                ...
                llm_risk=llm_risk,
                ae_error=ae_error,
                tau=15.0,  # Stage 7: 固定默认 tau, PPO 不再控制
                theta=0.7,  # 临时, (D) 步后覆盖
            )
        except Exception as e:
            log.warning(f"V31Engine 失败: {e}，使用等权")
        ...
        # Stage 7: tau 由 AE regime 决定, PPO 不再控制. 沿用 V3.1 默认 15.0
        tau_t = 15.0
        ...
        # ── (H) 本周组合收益 ─────────────────────────────────────────────
        # Stage 7: 单轨, 没有 w_normal. r_normal 设为 0 占位 (verify 脚本不依赖)
        r_normal = 0.0
        r_event = float(np.dot(w_event, week_returns))
        edge_event_minus_normal = r_event  # Stage 7: 与 0 比较等价于 r_event
