...
class EventTrackV31:
    ...
    # Default sensitivity (used when compute() called without theta).
    # Stage 6: 1.0 -> 0.7 — 减 sharp, 让 weekly std 回升, edge std 提升.
    # Stage 7: THETA 不再是 class 静态, 改为 compute() 参数, PPO 动态控制.
    ...
    def compute(
        ...
        """Stage 7: theta is a runtime parameter controlled by PPO.

        theta range: [0, 2]
          theta = 0    -> pure ERC (b0 uniform, no tilting)
          theta = 0.7  -> V3.1 default (Stage 6 calibrated)
          theta = 1.0  -> balanced
          theta = 2.0  -> aggressive concentration
        """
        if theta is None:
            theta = self.THETA_DEFAULT
        # Hard guard against out-of-range PPO outputs
        theta = float(np.clip(theta, 0.0, 2.0))
        ...
        # Exponential tilting (theta is PPO-controlled)
        b = self.B0 * np.exp(theta * scores)
        b_sum = b.sum()
