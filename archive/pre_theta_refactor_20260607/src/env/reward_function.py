...
class RewardFunction:
    ...
    def compute(
        ...
        # Regime-conditional alpha signal (Stage 1 fix).
        # In bull regime, high alpha (more EventTrack) is correct → reward.
        # In bear regime, high alpha is wrong (should use NormalTrack) → penalize.
        # This is the direct term the PPO controller learns from, complementary to
        # the regime-conditional switch_reward above.
        regime_sign = 1.0 if regime_bull else -1.0
        alpha_signal = self.lambda_alpha_direct * (alpha_current - 0.5) * regime_sign
        reward_t += alpha_signal
