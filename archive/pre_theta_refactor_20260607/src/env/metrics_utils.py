...
def calculate_tracking_error(
    ...
    """
    port_returns = np.asarray(port_returns, dtype=float).flatten()
    bench_returns = np.asarray(bench_returns, dtype=float).flatten()
    if port_returns.size < 2 or bench_returns.size < 2:
        return 0.0
