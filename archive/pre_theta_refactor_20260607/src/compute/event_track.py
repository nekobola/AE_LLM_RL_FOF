...
class EventTrack:
    ...
    def __init__(self, epsilon: float | None = None):
        ...
        self.base_neutral = np.array([0.20, 0.10, 0.30, 0.20, 0.20], dtype=float)
    ...
    def compute(
        ...
        Regime-conditional switch (Stage 2):
        - ae_error > tau: forced crisis prototype (no softmax)
        - growth_score > 1.2 AND equity_stress < 0.3: forced growth prototype
        - else: softmax blend over three prototypes
        """
        ...
        # Regime-conditional prototype switch
        if ae_error is not None and tau is not None and ae_error > tau:
            # Bear regime: 直接返回 crisis 原型,完全跳过 base_neutral 混合
            w_event = self.prototype_crisis.copy()
            regime_mode = "forced_crisis"
        elif growth_score > 1.2 and equity_stress < 0.3:
            # 强增长 + 低权益 stress: 直接返回 growth 原型
            w_event = self.prototype_growth.copy()
            regime_mode = "forced_growth"
        else:
            # Soft 混合: softmax 三个原型 + event_intensity 与 base_neutral 插值
            regime_mode = "softmax_blend"
        ...
        if regime_mode == "softmax_blend":
            logger.debug(
                "[EventTrack] mode=%s macro=%.1f sent=%.1f risk=%.1f intensity=%.2f proto=%s weights=%s",
                regime_mode,
                llm_macro,
                llm_sentiment,
                llm_risk,
            ...
            logger.debug(
                "[EventTrack] mode=%s macro=%.1f sent=%.1f risk=%.1f weights=%s",
                regime_mode,
                llm_macro,
                llm_sentiment,
                llm_risk,
