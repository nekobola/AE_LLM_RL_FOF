...
12. **Stage 7+ 待办**:
    - V3.1 live test 1 个月, 监控 Sharpe / max DD / 换手率 (实盘 vs 真实 OHLC 回测 baseline)
    - supervised regression 校准 V3.1 的 $\mathbf{W}$ 矩阵 + $\mathbf{V}_{\text{DEFENSE}}$ + ActionMapper 参数
    - PPO 仍锁在 alpha=1.0, 这是真实 PPO 在该样本上学到的稳态 — Stage 7 可考虑在 reward 上加更激进的 alpha 离散化 (例如 hard penalty at alpha=0.95+)
