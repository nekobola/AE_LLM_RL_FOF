...
def compute_weekly_return(price_df: pd.DataFrame, window: int = RETURN_WINDOW) -> pd.DataFrame:
    ...
    """
    ret = price_df.pct_change(periods=window, fill_method=None)
    return ret
...
def compute_momentum_20d(price_df: pd.DataFrame, window: int = MOMENTUM_WINDOW) -> pd.DataFrame:
    ...
    仅使用截至 t-1 的已确认价格。
    """
    momentum = price_df.pct_change(periods=window, fill_method=None).shift(1)
    return momentum
...
def compute_mean_corr_20d(price_df: pd.DataFrame, window: int = CORR_WINDOW) -> pd.DataFrame:
    ...
    # 注意：只对 price_df 中实际存在的资产列生成特征
    result = pd.DataFrame(
        {code: mean_corr_series for code in price_df.columns},
        index=returns.index,
    )
    return result
...
def compute_asset_features(
    ...
    # 保留已有列，过滤掉不存在的资产
    available_codes = [c for c in asset_codes if c in price_df.columns]
    if len(available_codes) < len(asset_codes):
        missing = set(asset_codes) - set(available_codes)
        import warnings
        warnings.warn(f"部分资产数据缺失，跳过: {missing}", RuntimeWarning)
    if not available_codes:
        raise ValueError("无可用的资产数据")
    price_df = price_df[available_codes].copy()
