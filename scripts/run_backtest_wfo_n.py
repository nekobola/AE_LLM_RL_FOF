...
# Stage 7c: 8-ETF version
from src.compute.v31_engine_n import V31EngineN
from src.compute.event_track_v3_1_n import EventTrackV31N
from src.selection.select_8_n import Select8N, fallback_8_etf_scores
...
import yaml
# Stage 7: NormalTrack + dual_track_engine 已删除, 见 archive/pre_theta_refactor_20260607/
# 使用 V31Engine 单轨
# from src.compute.dual_track_engine import DualTrackEngine  # Stage 6 旧 import
...
# ── 8 个 ETF (Stage 7c) ────────────────────────────────────────────────────
# Asset order matches V31EngineN.ASSET_CODES:
#   0: 511010 (国债 ETF)
#   1: 518880 (黄金 ETF)
#   2: 511020 (信用债 ETF)
#   3: 159985 (商品 ETF)
#   4: 512100 (中证1000 ETF)
#   5: 515050 (红利低波 ETF)
#   6: 159915 (创业板 ETF, Phase 2 替换 159919)
#   7: 510300 (沪深300 ETF)
ASSET_CODES = [
    "511010",  # 国债 ETF
    "518880",  # 黄金 ETF
    ...
    "512100",  # 中证1000 ETF
    "515050",  # 红利低波 ETF
    "159915",  # 创业板 ETF (Phase 2 替换 159919)
    "510300",  # 沪深300 ETF
]
...
def compute_rolling_b0(
    week_ts: pd.Timestamp,
    etf_returns_by_week: dict,
    lookback_weeks: int = 104,
    use_erc: bool = True,
) -> np.ndarray | None:
    """Compute pure data-driven B0 from rolling 8-ETF weekly returns.

    No Bayesian shrinkage, no hand-tuned prior. B0 is 100% derived from the
    rolling covariance of 8 ETF weekly returns. Engineering: clean, no
    look-ahead, fully reproducible.

    Two modes for the data-driven component:
      1. ERC (Equal Risk Contribution):  min_w sum (log w - log Σw)^2
      2. Inverse-volatility:             w_i = 1/σ_i

    Returns None if insufficient data (< 26 weeks), in which case V3.1N
    falls back to its hand-tuned B0 default.
    """
    available = sorted([k for k in etf_returns_by_week.keys() if k <= week_ts])
    if len(available) < 26:
        return None
    window_keys = available[-lookback_weeks:]
    returns = np.array([etf_returns_by_week[k] for k in window_keys])  # (T, 8)
    if returns.shape[0] < 26 or not np.isfinite(returns).all():
        return None
    from src.compute.event_track_v3_1_n import EventTrackV31N
    return EventTrackV31N.compute_b0_from_returns(returns, use_erc=use_erc)


def precompute_features_5d_by_week(
    etf_returns_by_week: dict,
    features_df: pd.DataFrame | None,
    llm_scores_df: pd.DataFrame | None,
    lookback_weeks: int = 5,
) -> dict:
    """Pre-compute 5-dim market features f_t for every week in etf_returns_by_week.

    f_t = (m_t, s_t, r_t, equity_stress_t, sat_lead_t):
      m, s, r: LLM-derived (50 fallback if missing)
      equity_stress = clip((σ_eq1+σ_eq2)/(σ_fi+σ_saf) - 1, 0, 2) / 2
      sat_lead = clip((σ_eq2-σ_eq1)/(σ_eq2+σ_eq1), -1, 1)
    where σ_· = std of 5 ref asset weekly returns over lookback_weeks weeks.

    Reference assets: 000300.SH (broad), 000852.SH (satellite),
                      CBA02701.CS (bond), AU9999.SGE (gold), NH0100.NHF (commodity)

    Returns
    -------
    dict[pd.Timestamp, np.ndarray]  of {week_end: f_5d array (5,)}
    """
    ref_cols = [
        "000300.SH__weekly_return",
        "000852.SH__weekly_return",
        "CBA02701.CS__weekly_return",
        "AU9999.SGE__weekly_return",
        "NH0100.NHF__weekly_return",
    ]
    if features_df is None or not all(c in features_df.columns for c in ref_cols):
        return {}

    out: dict = {}
    for week_end in sorted(etf_returns_by_week.keys()):
        available = [d for d in features_df.index if d <= week_end]
        if len(available) < lookback_weeks:
            continue
        window = available[-lookback_weeks:]
        try:
            rets = features_df.loc[window, ref_cols].values  # (T, 5)
        except (KeyError, TypeError):
            continue
        if rets.shape[0] < lookback_weeks or not np.isfinite(rets).all():
            continue

        sigmas = np.std(rets, axis=0, ddof=1)
        sigmas = np.where(sigmas < 1e-9, 1e-3, sigmas)

        equity_stress = float(
            np.clip((sigmas[0] + sigmas[1]) / (sigmas[2] + sigmas[3] + 1e-9) - 1.0, 0.0, 2.0)
        ) / 2.0
        sat_lead = float(
            np.clip((sigmas[1] - sigmas[0]) / (sigmas[1] + sigmas[0] + 1e-9), -1.0, 1.0)
        )

        m, s, r = 50.0, 50.0, 50.0
        if llm_scores_df is not None and week_end in llm_scores_df.index:
            row = llm_scores_df.loc[week_end]
            if hasattr(row, "d1") and not pd.isna(row.d1):
                m = float(row.d1)
            if hasattr(row, "d2") and not pd.isna(row.d2):
                s = float(row.d2)
            if hasattr(row, "d3") and not pd.isna(row.d3):
                r = float(row.d3)

        f = np.array(
            [
                float(np.clip((m - 50.0) / 50.0, -1.0, 1.0)),
                float(np.clip((s - 50.0) / 50.0, -1.0, 1.0)),
                float(np.clip((r - 50.0) / 50.0, -1.0, 1.0)),
                equity_stress,
                sat_lead,
            ],
            dtype=float,
        )
        out[pd.Timestamp(week_end)] = f
...
def compute_w_hybrid(
    week_ts: pd.Timestamp,
    features_5d_by_week: dict,
    etf_returns_by_week: dict,
    v31_engine,
    lookback_weeks: int = 104,
) -> np.ndarray | None:
    """Compute Phase 2 hybrid W matrix from rolling OLS.

    W_hybrid = sign(W_SIGN) * |D_scale|, where D_scale = OLS(etf_returns, f_t).

    Returns None if insufficient data; caller falls back to class W default.
    """
    available = sorted([k for k in etf_returns_by_week.keys() if k <= week_ts])
    if len(available) < 26:
        return None
    ...
    for k in window_keys:
        if k in features_5d_by_week:
            feats.append(features_5d_by_week[k])
            rets.append(etf_returns_by_week[k])
    if len(feats) < 26:
        return None
    features_window = np.array(feats, dtype=float)  # (T, 5)
    returns_window = np.array(rets, dtype=float)    # (T, 8)
    return v31_engine.compute_w_hybrid(features_window, returns_window)
...
def run_wfo(
    ...
    # ── 子系统初始化 ──────────────────────────────────────────────────────
    # Stage 7c: 8-ETF single-track V3.1N
    v31_engine = V31EngineN(config)
    et_version = config.get("event_track_version", "v3_1_n")
    selector_n = Select8N(config)
    log.info(f"[Stage 7c] 8-ETF single-track V3.1N -> {type(v31_engine.event_track).__name__}")
    veto_switch = VetoSwitch(config)
    action_mapper = ThetaActionMapper(
        theta_min=config.get("action_mapper", {}).get("theta_min", 0.0),
        theta_neutral=config.get("action_mapper", {}).get("theta_neutral", 1.0),
        theta_max=config.get("action_mapper", {}).get("theta_max", 2.0),
    )
    ...
    # ── Stage 7c: 8 ETF weekly returns (for r_event 真实计算) ────────────
    etf_returns_by_week = {}
    try:
        etf_df = fetch_n_etf(
        ...
        if not etf_df.empty:
            # 注意: etf_daily 的 LatestClosePrice 是前日 close, etf_day 的 adj_factor 是 1.0 浮动值
            # 两个表都不是真正的复权因子, 直接用 close 价 (已经是 adjusted price)
            etf_pivot = etf_df.pivot_table(index="date", columns="code", values="close").sort_index()
            etf_weekly = etf_pivot.resample("W-FRI").last().pct_change().dropna()
            for week_end, row in etf_weekly.iterrows():
                etf_returns_by_week[pd.Timestamp(week_end)] = row.values  # 8-dim
            log.info(f"8-ETF weekly returns: {len(etf_returns_by_week)} 周")
        else:
            log.warning("8-ETF 数据为空，r_event 将用 0 代替")
    except Exception as e:
        log.warning(f"8-ETF 加载失败: {e}")
    ...
    # ── 加载 walk-forward PPO checkpoints (Stage 7c C 选项) ─────────────────
    # {quarter_key: (ckpt_path, quarter_label)}  按 (year, quarter) 排序
    wf_ckpts: dict = {}
    if walkforward_dir and walkforward_dir.exists():
        import re
        pattern = re.compile(r"actor_critic_q(\d+)_(\d{4})\.pth")
        for ckpt_path in sorted(walkforward_dir.glob("actor_critic_q*_*.pth")):
            m = pattern.search(ckpt_path.name)
            if m:
                q, y = int(m.group(1)), int(m.group(2))
                wf_ckpts[(y, q)] = (ckpt_path, f"q{q}_{y}")
        log.info(f"Walk-forward PPO checkpoints: {len(wf_ckpts)} 个")
        for (y, q), (p, lbl) in sorted(wf_ckpts.items()):
            log.info(f"  {lbl}: {p.name}")
    else:
        log.info("未指定 walkforward_dir, 使用单 PPO checkpoint")
    ...
    nav = 1.0
    hwm = 1.0
    # Stage 7: 单轨, 用 theta_prev 替代 alpha_prev / tau_prev
    theta_prev = config.get("env", {}).get("initial_theta", 1.0)
    w_fused_prev = np.array([0.2] * 5)
    last_retrain_quarter = None  # 避免同一季度重复重训
    current_wf_ckpt_path: Path | None = None  # Walk-forward 当前加载的 checkpoint

    # Phase 2: pre-compute 5-dim market features for every week
    features_5d_by_week = precompute_features_5d_by_week(
        etf_returns_by_week, features_df, llm_scores_df, lookback_weeks=5
    )
    log.info(f"5-dim features pre-computed: {len(features_5d_by_week)} 周")
    ...
    for i, friday in enumerate(fridays):
        ...
        # Stage 7c fix: ae_error 在 (C) 步 v31_engine.compute() 之前就要确定
        # 之前在 (D) 步才定义, 导致第一周 UnboundLocalError 走 fallback 等权
        ae_error = 0.0
        if features_df is not None:
            try:
        ...
        # Phase 1: Bayesian-shrunk B0 (50/50 data + hand-tuned prior)
        # Phase 1: pure data-driven B0 (ERC over rolling 104w 8-ETF returns)
        b0_t = compute_rolling_b0(week_ts, etf_returns_by_week, lookback_weeks=104)
        # Phase 2: hybrid W = sign(W_SIGN) * |D_scale|, D_scale from rolling 104w OLS
        w_hybrid_t = compute_w_hybrid(
            week_ts, features_5d_by_week, etf_returns_by_week, v31_engine, lookback_weeks=104,
        )
        # Phase 3: data-driven tau (rolling 104w E_t 的 70 分位, 让 30% 周进入 bear regime)
        if features_df is not None and "reconstruction_error" in features_df.columns:
            available = [d for d in features_df.index if d <= week_ts]
            if len(available) >= 26:
                window = available[-104:]
                e_window = features_df.loc[window, "reconstruction_error"].dropna().values
                tau_t = EventTrackV31N.compute_tau_from_ae_errors(e_window, quantile=0.30)
                if i < 5 or i % 26 == 0:  # 偶尔 log
                    log.info(f"  Phase 3 tau: {tau_t:.2f}  (E_t window: mean={np.mean(e_window):.2f}, p70={tau_t:.2f})")
            else:
                tau_t = None
        else:
            tau_t = None
        try:
            ...
            # Stage 7c: 8-ETF V3.1N, 5 维 returns_5d 喂入 V3.1 feature (m, s, r, eq, sat_lead)
            # V3.1N 内部用这 5 维算 8 资产权重
            w_event = v31_engine.compute(
                ...
                ae_error=ae_error,
                tau=tau_t,
                theta=0.7,  # (D) 步 PPO 推理后覆盖
                b0=b0_t,
                w_matrix=w_hybrid_t,
            )
        except Exception as e:
            log.warning(f"V31EngineN 失败: {e}，使用等权")
            w_event = np.array([0.125] * 8)
        ...
        # ae_error 已在 (C) 步提取 (line 472-485), 此处不再重复

        # ── Walk-forward PPO 切换: 找到本季度对应的 checkpoint, 必要时 hot-swap ──
        if ac is not None and wf_ckpts:
            week_quarter = (friday.year, (friday.month - 1) // 3 + 1)
            # 找最新的 <= 本季度的 checkpoint
            available_qs = sorted([q for q in wf_ckpts.keys() if q <= week_quarter])
            if available_qs:
                target_q = available_qs[-1]
                target_path, target_label = wf_ckpts[target_q]
                if target_path != current_wf_ckpt_path:
                    try:
                        new_ckpt = torch.load(target_path, map_location="cpu", weights_only=True)
                        ac.load_state_dict(new_ckpt["ac"])
                        ac.eval()
                        current_wf_ckpt_path = target_path
                        log.info(f"  ⟳ Week {week_label} 切换 walk-forward PPO → {target_label}")
                    except Exception as e:
                        log.warning(f"  Walk-forward 切换失败: {e}, 沿用上一个")
        ...
        if ac is not None and state_assembler is not None:
            try:
                ...
                # Stage 7: 9-dim state with theta_prev
                ...
                with torch.no_grad():
                    ...
                    a = float(action_mean[0].numpy()[0])
                    theta_t = action_mapper.map(a)

                if np.isnan(theta_t):
                    raise ValueError("Actor output degenerate")

                log.debug(f"  Actor: mom={mom_20d:.2f}%, action=({a:.3f}), theta={theta_t:.3f}")

            except Exception as e:
                log.debug(f"  Actor failed: {e}, using momentum fallback")

        # Fallback: Direct Momentum-Based Theta Policy
        if theta_t is None:
            THETA_HIGH = 1.5
            THETA_LOW = 0.3
            MOM_REF = 5.0
            mom_norm = float(np.clip(mom_20d / MOM_REF, -1.0, 1.0))
            theta_t = float(np.clip(0.9 + mom_norm * 0.4, THETA_LOW, THETA_HIGH))
            if features_df is not None:
                try:
                    if week_ts in features_df.index:
                        row = features_df.loc[week_ts]
                    ...
                    vol_zscore = 0.0
                if vol_zscore > 1.0:
                    theta_t = float(np.clip(theta_t - 0.3, THETA_LOW, THETA_HIGH))
            log.debug(f"  Momentum fallback: mom={mom_20d:.2f}%, theta={theta_t:.3f}")

        # Force theta override (for diagnostic testing)
        if force_theta is not None:
            theta_t = float(np.clip(force_theta, 0.0, 2.0))
            log.debug(f"  Force theta: {theta_t:.3f}")

        # ── (E) Stage 7: 单轨 V3.1, w_fused = w_event (PPO theta 在 v31_engine.compute 内部已应用) ──
        # 实际用 PPO 算出的 theta 重新调 v31_engine:
        try:
            w_event = v31_engine.compute(
                returns_5d,
                llm_macro=llm_macro,
            ...
            log.warning(f"V31Engine 重算失败: {e}, 沿用 (C) 步结果")
        w_fused = np.clip(w_event, 0, 1)
        w_fused = w_fused / (w_fused.sum() + 1e-9)

        # ── (E) Stage 7: 单轨 V3.1, w_fused = w_event (PPO theta 在 v31_engine.compute 内部已应用) ──
        # 在 v31_engine 替换 dual_engine 后, w_event 已经是 PPO 控 theta 下的最优权重.
        # 旧 alpha*w_event + (1-alpha)*w_normal 公式废弃.
        w_fused = np.clip(w_event, 0, 1)
        w_fused = w_fused / (w_fused.sum() + 1e-9)

        # ── (F) Regret + 8 ETF 真实 weekly returns ─────────────────────────
        # 5 维 synthetic features (from features_master) 给 V3.1 算 score
        if features_df is not None:
            if week_ts in features_df.index:
                week_returns_synth = (
                ...
                if available:
                    ...
                    week_returns_synth = np.random.randn(5) * 0.01
        else:
            week_returns_synth = np.random.randn(5) * 0.01
        week_returns_synth = np.nan_to_num(week_returns_synth, nan=0.0)

        # 8 维 ETF 真实 weekly returns (从 fetch_n_etf 拉的)
        if week_ts in etf_returns_by_week:
            etf_rets_8d = etf_returns_by_week[week_ts]
        else:
            if i < 3: log.info(f"DEBUG: week_ts={week_ts} (type {type(week_ts).__name__}) NOT in dict, sample keys: {list(etf_returns_by_week.keys())[:3]}")
            etf_rets_8d = np.array([0.0] * 8)

        try:
            _, regret_norm = regret_engine.compute(w_fused_prev, etf_rets_8d)
        except Exception:
            regret_norm = 0.0
        ...
        # ── (H) 本周组合收益 ─────────────────────────────────────────────
        # Stage 7c: 8 维 w_event × 8 ETF 真实 weekly returns
        r_normal = 0.0
        r_event = float(np.dot(w_event, etf_rets_8d))
        edge_event_minus_normal = r_event
        regime_label = "bull_normal" if ae_error < tau_t else "event_stress"
        week_return = float(np.dot(w_fused, etf_rets_8d))
        ...
        gate_records.append({
            ...
            "regret_ema_norm": regret_norm,
            "w_event_0": float(w_event[0]),
            "w_event_1": float(w_event[1]),
            "w_event_2": float(w_event[2]),
            "w_event_3": float(w_event[3]),
            "w_event_4": float(w_event[4]),
            "w_event_5": float(w_event[5]),
            "w_event_6": float(w_event[6]),
            "w_event_7": float(w_event[7]),
            "w_fused_0": float(w_fused[0]),
            "w_fused_1": float(w_fused[1]),
            "w_fused_2": float(w_fused[2]),
            "w_fused_3": float(w_fused[3]),
            "w_fused_4": float(w_fused[4]),
            "w_fused_5": float(w_fused[5]),
            "w_fused_6": float(w_fused[6]),
            "w_fused_7": float(w_fused[7]),
        })
        ...
        if (i + 1) % 52 == 0:
            log.info(
                ...
                f"theta={theta_t:.3f}  regret={regret_norm:.3f}"
            )
    ...
    with open(tearsheet, "w") as f:
        ...
        for k, v in metrics.items():
            try:
                ...
                f.write(f"  {k}: {v}\n")
        f.write(f"\n── 策略参数 ──────────────────────────────\n")
        f.write(f"  theta (V3.1 偏置): mean={df_nav['theta'].mean():.3f}, std={df_nav['theta'].std():.3f}\n")
    ...
    log.info("══ 集成 GeneralBacktest 绘图 (8 ETF OHLC) ═")
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
        from GeneralBacktest import GeneralBacktest
        # fetch_n_etf already imported at module level (line 25)

        # 1. 从 ClickHouse 拉 8 ETF 日频 OHLC
        etf_raw = fetch_n_etf(
            start_date=start_date,
            end_date=end_date,
            columns=["open", "close"],
        )
        # 转换为 GeneralBacktest 格式 (melt)
        # adj_factor = 1.0 (实际是 close 价已经是 adjusted price, 不需要 * adj_factor)
        records_for_gbt = []
        for _, row in etf_raw.iterrows():
            date_str = row["date"]
            code = row["code"]
            o = row["open"]
            c = row["close"]
            if pd.isna(c) and pd.isna(o):
                continue
            final_close = float(c) if not pd.isna(c) else float(o)
            final_open = float(o) if not pd.isna(o) else final_close
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="强制指定theta值（0=纯ERC, 1=平衡, 2=激进），覆盖Actor决策",
    )
    parser.add_argument(
        "--walkforward-dir",
        default="checkpoints/walkforward",
        help="walk-forward PPO checkpoint 目录 (含 actor_critic_q{N}_{year}.pth)",
    )
    parser.add_argument(
        "--ppo-checkpoint",
        default=None,
        help="指定 PPO checkpoint 路径, 默认 checkpoints/actor_critic.pth",
    )
    ...
    ppo_ckpt = (
        PROJECT_ROOT / args.ppo_checkpoint
        if args.ppo_checkpoint
        else PROJECT_ROOT / paths_cfg.get("checkpoints", "checkpoints") / "actor_critic.pth"
    )
    wf_dir = PROJECT_ROOT / args.walkforward_dir
    ...
    try:
        df = run_wfo(
            ...
            walkforward_dir=wf_dir if wf_dir.exists() else None,
            use_wfo_scheduler=not args.no_scheduler,
            force_theta=args.force_theta,
        )
        log.info("WFO 回测完成")
    except Exception as e:
