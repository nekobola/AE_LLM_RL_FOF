...
数据源：ClickHouse etf_day (2005年至今)
  - 510300 沪深300ETF
...
调度链路：
  1. 从 ClickHouse 拉取 ETF 日频量价数据
  2. 调用 asset_features 计算 5资产 x 4特征 = 20维资产特征
...
def run_etl(
    ...
    # ── Step 1: 拉取 ClickHouse ETF 数据 ─────────────────────────────────
    log.info("[1/5] 拉取 ClickHouse ETF 数据 ...")
    try:
        df_etf = fetch_track_b_safe(start_date=start_date, end_date=end_date)
        log.info(f"    ETF 数据拉取完成: {len(df_etf)} 条")
    except Exception as e:
        log.error(f"ETF 数据拉取失败: {e}")
        raise

    # ── Step 2: 计算资产特征 (5资产 × 4特征 = 20维) ─────────────────────────
    log.info("[2/5] 计算资产特征 (momentum/volatility/mean_corr/weekly_return) ...")
    feat_cfg = config.get("features", {})
    asset_feat = compute_asset_features(df_etf)
    log.info(f"    资产特征: {asset_feat.shape}, 列数: {len(asset_feat.columns)}")

    # ── Step 3: 计算宏观特征 (5维) ───────────────────────────────────────────
    log.info("[3/5] 计算宏观特征 (DR007/汇率/国债/利差/北向) ...")
    ...
    # ── Step 4: 对齐时间轴，拼接25维特征矩阵 ────────────────────────────────
    log.info("[4/5] 对齐时间轴，拼接25维特征矩阵 ...")
    ...
    # ── Step 5: 严格 Z-score 标准化 (防穿越) ─────────────────────────────────
    log.info("[5/5] 执行严格Z-score标准化 (窗口252日，防穿越) ...")
