...
def load_config() -> dict:
    ...
    # 替换 config 中的 ${ENV_VAR} 为环境变量值
    import re
    def _sub_env(val):
        if isinstance(val, str):
            return re.sub(r"\$\{(\w+)\}", lambda m: os.environ.get(m.group(1), val), val)
...
def get_prior_scores(db_path: Path, week_end: str) -> dict[str, dict[str, float]]:
    """Load the prior week's scores for context. Returns {} if none exist."""
    prior_friday = (date.fromisoformat(week_end) - timedelta(days=7)).isoformat()
    conn = sqlite3.connect(db_path)
    df = pd.read_sql(
        f"SELECT concept, d1, d2, d3 FROM llm_scores "
        f"WHERE week_end = '{prior_friday}' AND error IS NULL",
        conn,
        index_col="concept",
    )
    conn.close()
    if df.empty:
        return {}
    return df.to_dict("index")  # {concept: {"d1": val, "d2": val, "d3": val}}
...
async def process_week(
    ...
    """
    prior_scores = get_prior_scores(db_path, week_end)
    try:
        scores = await engine.evaluate(week_end, prior_scores)
        insert_scores(db_path, scores, week_end)
        log.info(f" {week_end}: {list(scores.keys())}")
        return True
    except LLMCallError as e:
        mark_error(db_path, week_end, str(e))
        log.warning(f"  WARN  {week_end}: LLM失败 -> checkpoint错误，继续: {e}")
        return False
    except Exception as e:
        mark_error(db_path, week_end, str(e))
        log.error(f"  FAIL  {week_end}: 未知错误: {e}")
        return False
...
async def run_batch(
    ...
    log.info(f"Batch 完成: OK={n_ok} FAIL={n_fail}")
